class ZCL_ZGW_CASH_FLOW_DPC definition
  public
...
protected section.
  ...
  methods CASHFLOWSET_CREATE_ENTITY
    importing
    ...
    exporting
      !ER_ENTITY type ZCL_ZGW_CASH_FLOW_MPC=>TS_CASHFLOW
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods CASHFLOWSET_DELETE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods CASHFLOWSET_GET_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
  ...
  methods CASHFLOWSET_GET_ENTITYSET
    ...
    exporting
      !ET_ENTITYSET type ZCL_ZGW_CASH_FLOW_MPC=>TT_CASHFLOW
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods CASHFLOWSET_UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
  ...
  methods DBUKNSET_CREATE_ENTITY
    importing
      ...
      !IO_DATA_PROVIDER type ref to /IWBEP/IF_MGW_ENTRY_PROVIDER optional
    exporting
      !ER_ENTITY type ZCL_ZGW_CASH_FLOW_MPC=>TS_DBUKN
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  ...
  methods DBUKNSET_GET_ENTITYSET
    importing
      ...
      !IO_TECH_REQUEST_CONTEXT type ref to /IWBEP/IF_MGW_REQ_ENTITYSET optional
    exporting
      !ET_ENTITYSET type ZCL_ZGW_CASH_FLOW_MPC=>TT_DBUKN
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
...
CLASS ZCL_ZGW_CASH_FLOW_DPC IMPLEMENTATION.


  method /IWBEP/IF_MGW_APPL_SRV_RUNTIME~CREATE_ENTITY.
*&----------------------------------------------------------------------------------------------*
*&  Include           /IWBEP/DPC_TEMP_CRT_ENTITY_BASE
*&* This class has been generated on 28.05.2018 23:50:04 in client 100
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_ZGW_CASH_FLOW_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA dbuknset_create_entity TYPE zcl_zgw_cash_flow_mpc=>ts_dbukn.
 DATA cashflowset_create_entity TYPE zcl_zgw_cash_flow_mpc=>ts_cashflow.
...
*-------------------------------------------------------------------------*
*             EntitySet -  CashFlowSet
*-------------------------------------------------------------------------*
     WHEN 'CashFlowSet'.
*     Call the entity set generated method
    cashflowset_create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
                   iv_entity_set_name = iv_entity_set_name
                   iv_source_name     = iv_source_name
                   io_data_provider   = io_data_provider
                   ...
                   it_navigation_path = it_navigation_path
                   io_tech_request_context = io_tech_request_context
       	 IMPORTING er_entity          = cashflowset_create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
        is_data = cashflowset_create_entity
      CHANGING
        cr_data = er_entity
   ).
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA dbuknset_get_entity TYPE zcl_zgw_cash_flow_mpc=>ts_dbukn.
 DATA cashflowset_get_entity TYPE zcl_zgw_cash_flow_mpc=>ts_cashflow.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
          cashflowset_get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = cashflowset_get_entity
                         es_response_context = es_response_context
          ).

        IF cashflowset_get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = cashflowset_get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*&*   generated methods inside the DPC provider subclass - ZCL_ZGW_CASH_FLOW_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA dbuknset_get_entityset TYPE zcl_zgw_cash_flow_mpc=>tt_dbukn.
 DATA cashflowset_get_entityset TYPE zcl_zgw_cash_flow_mpc=>tt_cashflow.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
      cashflowset_get_entityset(
        EXPORTING
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = cashflowset_get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = cashflowset_get_entityset
        CHANGING
          cr_data = er_entityset
      ).
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA dbuknset_update_entity TYPE zcl_zgw_cash_flow_mpc=>ts_dbukn.
 DATA cashflowset_update_entity TYPE zcl_zgw_cash_flow_mpc=>ts_cashflow.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
*     Call the entity set generated method
          cashflowset_update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = cashflowset_update_entity
          ).
       IF cashflowset_update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = cashflowset_update_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
* Call to Search Help run time mechanism to get values
  ...
  method CASHFLOWSET_CREATE_ENTITY.
  ...
  method CASHFLOWSET_DELETE_ENTITY.
  RAISE EXCEPTION TYPE /iwbep/cx_mgw_not_impl_exc
    EXPORTING
      textid = /iwbep/cx_mgw_not_impl_exc=>method_not_implemented
      method = 'CASHFLOWSET_DELETE_ENTITY'.
  ...
  method CASHFLOWSET_UPDATE_ENTITY.
  RAISE EXCEPTION TYPE /iwbep/cx_mgw_not_impl_exc
    EXPORTING
      textid = /iwbep/cx_mgw_not_impl_exc=>method_not_implemented
      method = 'CASHFLOWSET_UPDATE_ENTITY'.
