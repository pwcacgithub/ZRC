FUNCTION zfm_fi_payment_details.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     VALUE(COMPANYCODE) TYPE  ZFIST_BUKRS
*"     VALUE(FISCALYEAR) TYPE  GJAHR
*"     VALUE(POSTINGDATE) TYPE  ZFIST_DATES OPTIONAL
*"  EXPORTING
...
*"----------------------------------------------------------------------
  ...
  RANGES : r_bukrs FOR t001-bukrs,
           r_budat FOR acdoca-budat.


  r_bukrs-sign = companycode-sign.
  r_bukrs-option = companycode-option.
  r_bukrs-low = companycode-low.
  r_bukrs-high = companycode-high.

  APPEND r_bukrs .

  r_budat-sign = postingdate-sign.
  r_budat-option = postingdate-option.
  r_budat-low = postingdate-low.
  r_budat-high = postingdate-high.
